...
def choose_action(state, model_predict_fn, horizon, n_samples, environment_forecast, curr_step, model, epsilon, winter=True):
    ...
    for t in range(horizon):
        ...
        next_zone_temperature_vector, _ = model_predict_fn(state_vector, model)
...
'''The reward function'''
def evaluate_rewards(zone_temperature_matrix, action_matrix, env_forecast, winter, gamma=0.99):
...
run_experiment(days, 
               ...
               monitor_path='{}_GPMMPC_winter_{}_{}_monitor.csv'.format(city, days, eps), 
               overhead_path='{}_GPMMPC_winter_{}_{}_overhead.csv'.format(city, days, eps), 
               epsilon=eps, 
               winter=True)